...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function killStackTriggers() {
    ...
    // Remove todos os ScrollTriggers com id 'stack-pin' (caso múltiplos)
  ...
  function initStackAnimation() {
    ...
    // Timeline para empilhar os cards
    ...
    // ScrollTrigger para pin e animação
    stackPinTrigger = ScrollTrigger.create({
      ...
      onLeave: () => {
        // Garante que todos os cards estejam visíveis ao sair do pin
        cards.forEach(card => gsap.set(card, { y: 0, opacity: 1 }));
      },
      onEnterBack: () => {
        // Reaplica o reset ao voltar
        resetCards();
      }
    });
  }
  ...
  if (stackWrapper && cards.length > 0) {
    // Trigger para garantir que a animação sempre reinicialize ao entrar no viewport
